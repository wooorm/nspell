 ...
 * @license MIT
 * @module nspell:util:dictionary
 * @fileoverview Parse a dictionary.
 */
 ...
 *
 * @private
 * @param {Buffer} buf - Buffer of dictionary.
 * @param {Object} options - Configuration.
 * @param {Object} dict - Object to patch new words
 *   and rules on.
 */
function parse(buf, options, dict) {
  ...
  while (index !== -1) {
    if (value.charCodeAt(last) !== CC_TAB) {
      ...
      if (
        !has(flags, 'NEEDAFFIX') ||
        codes.indexOf(flags.NEEDAFFIX) === -1
      ) {
        add(word, codes);
      }
