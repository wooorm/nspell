 ...
 * @license MIT
 * @module nspell:suggest
 * @fileoverview Suggest spelling for `word`.
 */
...
function suggest(value) {
  ...
  while (++index < length) {
    suggestion = suggestions[index] = normalize(suggestions[index], conversion.out);
 ...
 *
 * @param {NSpell} context - Context object.
 * @param {Object} memory - Memory to use.
 * @param {Object} memory.state - State to store whether
 *   words are correctly spelled or not.  Suggested words
 *   are weighted based on how many ways they are
 *   generated.  Caching whether a word os correctly
 *   spelled thus speeds this process up.
 * @param {Object} memory.weighted - Place to store number
 *   of times correctly spelled words are generated.
 * @param {Array} memory.suggestions - List of suggested
 *   words: correctly generated words.
 * @param {Array.<string>} words - List of words.
 * @param {Array.<string>} edits - List of words to
 *   handle as if they had an edit-distance of one.
 *   These are double-weighted.
 * @return {Array.<string>} - List of close, possible,
 *   words.
 */
function generate(context, memory, words, edits) {
   ...
   * @param {boolean?} [double] - When `value` is real
   *   and new, double its weight.
   */
